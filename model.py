...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # On applique sin à tous les indices pairs et cos à tous les indices impairs
        pe[:, 0::2] = torch.sin(position * div_term)
        pe[:, 1::2] = torch.cos(position * div_term)

        # On ajoute la dimension batch (pour chaque échantillon d'entraînement)
        pe = pe.unsqueeze(0)
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x, src_mask):
        x = self.residual_connections[0](x, lambda x: self.self_attention_block(x, # Query
                                                                                x, # Key
                                                                                x, #Value
                                                                                src_mask)) # Mask
...
def build_transformer(src_vocab_size: int,
    ...
    transformer = Transformer(encoder, decoder, src_embed, target_embed, src_position, target_position, projection_layer)

    # Initialize the weights
    for p in transformer.parameters():
        if p.dim() > 1:
            nn.init.xavier_uniform_(p) # Je comprends pas exactement pourquoi c'est utile
